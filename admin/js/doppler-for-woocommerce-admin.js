...
(function ($) {
	...
	$(window).on("load", function () {
			...
			.change(function () {
				...
				if (prevData.val !== "") {
					mappingFieldsSelects.each(function () {
						if (
							checkFieldType(prevData.type, $(this).attr("data-type")) &&
							prevData.name !== $(this).attr("name")
						) {
							$(this).append(
								'<option value="' +
									prevData.val +
		...
		listsSelect
			...
			.change(function () {
				...
				$(this)
					.closest("tr")
					.find("td span")
					.html($("option:selected", this).attr("data-subscriptors"));
			});
		...
		function syncrhonizeLists(resp) {
			...
			$.when(
				createDefaultList(buyersList, "buyers"),
				createDefaultList(contactsList, "contacts")
			).done(function (bl, cl) {
		...
		$(".deactivate a").click(function (e) {
			if (
				$(this).closest("tr").attr("data-plugin") ==
				"doppler-for-woocommerce/doppler-for-woocommerce.php"
			) {
	...
	function checkFieldType(dplrType, wcType) {
		...
		if (
			$.inArray(wcType, types[dplrType]) !== -1 ||
			(dplrType === "string" && wcType === "")
		) {
			return true;
		}
	...
	function createDefaultList(list_id, list_type) {
		...
		if (list_id != "0") {
			...
			var listName = "";
			var selectElement = {};
			if (list_type != "buyers" && list_type != "contacts")
