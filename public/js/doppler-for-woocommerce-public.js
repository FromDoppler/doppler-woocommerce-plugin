(function ($) {
	...
	jQuery(document).ready(function ($) {
		...
		$(
			"#billing_email, #billing_phone, input.input-text, input.input-checkbox, textarea.input-text"
		).on("keyup keypress change", getCheckoutData); //All action happens on or after changing Email or Phone fields or any other fields in the Checkout form. All Checkout form input fields are now triggering plugin action. Data saved to Database only after Email or Phone fields have been entered.
			...
			if ($("#billing_email").length > 0) {
				...
				if (
					!(
						...
						dotposition + 2 >= dplrwoo_email.length
					)
				) {
					//If email is valid
					...
					var data = {
						action: "save_data",
						dplrwoo_nonce: dplrWooAjaxObj.nonce,
					...
					timer = setTimeout(function () {
						$.post(dplrWooAjaxObj.ajaxurl, data, function (response) {
